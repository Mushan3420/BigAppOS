...
public abstract class LoadingLayout extends FrameLayout implements
    ...
    private void initLayout(Context context, TypedArray attrs,
                            final Orientation scrollDirection, boolean footerCustom) {
        int layoutHorizontalId = R.layout.pull_to_refresh_default_horizontal;
        int layoutVerticalId = R.layout.pull_to_refresh_default_vertical;
        if (footerCustom
                && attrs.hasValue(R.styleable.PullToRefresh_prtLayoutFooter)) {
            LAYOUT_MODE layout_mode = LAYOUT_MODE.mapIntToValue(attrs
                    .getInteger(R.styleable.PullToRefresh_prtLayoutFooter, 0));
            switch (layout_mode) {
                case CUSTOM:
                    layoutHorizontalId = R.layout.pull_to_refresh_footer_horizontal_custom;
                    layoutVerticalId = R.layout.pull_to_refresh_footer_vertical_custom;
                    break;
                case NONE:
                    layoutHorizontalId = R.layout.pull_to_refresh_footer_horizontal_none;
                    layoutVerticalId = R.layout.pull_to_refresh_footer_vertical_none;
                    break;
                case SAME:
                    layoutHorizontalId = R.layout.pull_to_refresh_default_horizontal;
                    layoutVerticalId = R.layout.pull_to_refresh_default_vertical;
                    break;
                default:
                    break;
            }
        } else if (attrs.hasValue(R.styleable.PullToRefresh_prtLayoutHeader)) {
            LAYOUT_MODE layout_mode = LAYOUT_MODE.mapIntToValue(attrs
                    .getInteger(R.styleable.PullToRefresh_prtLayoutHeader, 0));
            switch (layout_mode) {
                case CUSTOM:
                    layoutHorizontalId = R.layout.pull_to_refresh_header_horizontal_custom;
                    layoutVerticalId = R.layout.pull_to_refresh_header_vertical_custom;
                    break;
                case NONE:
                    layoutHorizontalId = R.layout.pull_to_refresh_header_horizontal_none;
                    layoutVerticalId = R.layout.pull_to_refresh_header_vertical_none;
                    break;
                case SAME:
                    layoutHorizontalId = R.layout.pull_to_refresh_default_horizontal;
                    layoutVerticalId = R.layout.pull_to_refresh_default_vertical;
        ...
        switch (scrollDirection) {
            case HORIZONTAL:
                LayoutInflater.from(context).inflate(layoutHorizontalId, this);
                break;
            case VERTICAL:
            default:
                LayoutInflater.from(context).inflate(layoutVerticalId, this);
                break;
        }
    }
    ...
    public LoadingLayout(Context context, final Mode mode,
        ...
        this(context, mode, scrollDirection, attrs, false);
    }
    ...
    public LoadingLayout(Context context, final Mode mode,
        ...
        initLayout(context, attrs, scrollDirection, footerCustom);
    ...
    public final void onPull(float scaleOfLayout) {
        ...
        float scale = 360 - (float) scaleOfLayout * 360;

        if (sysVersion >= 11) {
            mHeaderImage.setRotation(scale);
        } else {
            Matrix matrix = new Matrix();
            mHeaderImage.setScaleType(ScaleType.MATRIX); // required
            matrix.postRotate(scale, 50, 50);
    ...
    public static enum LAYOUT_MODE {
        SAME(0x0), CUSTOM(0x1), NONE(0x2);
        private int mIntValue;

        static LAYOUT_MODE getDefault() {
            return SAME;
        }

        LAYOUT_MODE(int intValue) {
            mIntValue = intValue;
        }

        static LAYOUT_MODE mapIntToValue(final int layoutInt) {
            for (LAYOUT_MODE value : LAYOUT_MODE.values()) {
                if (layoutInt == value.getIntValue()) {
                    return value;
                }
            }
