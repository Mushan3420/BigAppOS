...
public class BaseImageDecoder implements ImageDecoder {
    ...
    private static ExifInfo getExifInfo(String path) {
        BaseImageDecoder decoder = new BaseImageDecoder(true);
        ExifInfo exif = decoder.defineExifOrientation(Scheme.FILE.wrap(path));
        return exif;
    }
    ...
    public static ImageSize getImageSize(String path) {
        ...
        if (options.outWidth > options.outHeight) {
            targetImageSize = new ImageSize(1280, 960,
                    getExifInfo(path).rotation, options.outMimeType);
        } else {
            targetImageSize = new ImageSize(960, 1280,
                    getExifInfo(path).rotation, options.outMimeType);
        }
