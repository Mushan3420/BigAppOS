...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = NSLocalizedString(@"DOWN_CAN_REFRESH", nil);
        self.releaseToRefreshText = NSLocalizedString(@"RELEASE_NOW_REFRESH", nil);
...
- (void)layoutSubviews
{
    ...
    // 1.状态标签
    self.statusLabel.frame = CGRectMake(statusX, statusY+12, statusWidth, statusHeight);
...
- (void)updateTimeLabel
{
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@：%@", NSLocalizedString(@"LAST_REFRESH", nil),time];
}
