...
#define RGBCOLORs(r,g,b)     [UIColor colorWithRed:r/255.0 green:g/255.0 blue:b/255.0 alpha:1]


#define FlatSkyBlue RGBCOLORs(166, 228, 193)
...
- (void)setType:(MozAlertType)type text:(NSString*)text doText:(NSString*)doText{
    ...
    [self setFrame:CGRectMake(0, -40, width, 44)];
    ...
    UILabel *textLabel = [[UILabel alloc]initWithFrame:CGRectMake((width - textLabelWidth)*0.5, 10, textLabelWidth, CGRectGetHeight(self.frame))];
...
- (void)show{
    dispatch_block_t showBlock = ^{
        [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.6 initialSpringVelocity:1 options:UIViewAnimationOptionCurveLinear animations:^{
            self.layer.position = CGPointMake(self.layer.position.x, self.layer.position.y + 34);
